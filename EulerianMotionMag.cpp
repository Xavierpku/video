...
void EulerianMotionMag::run()
{
    ...
    Mat grad_x,grad_y,abs_grad_x,abs_grad_y,grad,edge_mask, result, average;   
    Mat kernel_vibe = getStructuringElement(MORPH_RECT,Size(7,7));
    Mat kernel_dilate = getStructuringElement(MORPH_RECT,Size(11,11));
    Mat kernel_open = getStructuringElement(MORPH_RECT,Size(15,15));
    Mat previous_2,previous_1;
    //Define a global ROI to record the previous ROI
    ROI critical(0,0,0,0,0.0);
    vector<Vec4i> hierarchy;

    vector<vector<Point> > contours;
    vector<RotatedRect> box(200);
    while (1)
    {
        timer_.start();
	...
	{
		average = img_input_;
		previous_1 = img_input_.clone();
	}
	/*else if(frame_num_ == 1)
	{
		average = (previous_2+img_input_)/2;
		previous_1 = img_input_.clone();
	}*/
	else{
		average = (previous_1+img_input_)/2;
		//previous_2 = previous_1.clone();
		previous_1 = img_input_.clone();
	}
	//----------------------------------------------------------------------
	/*cvtColor(average,gray,CV_BGR2GRAY);
	...
	imshow("original edge",grad);

	threshold(grad,grad,40,255,CV_THRESH_BINARY);
        ...
        bitwise_and(edge_mask,gray,result);
        imshow("edge_free original",result);
	*/
	//----------------------------------------------------------------------

        // resize input image
	//resize(average,average, cv::Size(input_img_width_, input_img_height_));
	//resize(result, result, cv::Size(input_img_width_, input_img_height_));
	...
	else
	{
		...
		//erode(mask,mask,kernel_vibe);
		imshow("final points",mask);	
		//-----------vibe end----------------------------------------------------------------------
...
//	imshow("Edge detected by Sobel",grad);
	...
	dilate(grad,grad,kernel_dilate);
//	morphologyEx(grad,grad,MORPH_CLOSE,kernel_open);
//	morphologyEx(grad,grad,MORPH_OPEN,kernel_open);
//	imshow("dilate",grad);
	edge_mask = ~grad;
	imshow("edge mask",edge_mask);

	bitwise_and(edge_mask,mask,result);
	morphologyEx(result,result,MORPH_CLOSE,kernel_open);
	morphologyEx(result,result,MORPH_OPEN,kernel_open);
	imshow("vibe after edge mask",result);
	//-----------edge detection end-------------------------------------------------------------
	erode(result,result,kernel_vibe);
	dilate(result,result,kernel_dilate);
	//-----------select region start------------------------------------------------------------
	findContours(result, contours, hierarchy, CV_RETR_EXTERNAL, CV_CHAIN_APPROX_NONE);
	//contours.size is the number of connected region
	for(int i = 0; i < contours.size();i++)
	{

		box[i] = fitEllipse(Mat(contours[i]));
	//	ellipse(result,box[i],Scalar(255,0,0),4,8);
		circle(result,box[i].center,1,Scalar(0,0,255),4,8);
	}
	imshow("region after cluster",result);
	//-----------select region end--------------------------------------------------------------
     }
